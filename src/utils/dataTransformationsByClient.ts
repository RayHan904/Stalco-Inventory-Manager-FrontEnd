...
import { TotalOrderByClientData } from '../services/api';
...
//     };
// }
export function createSummary(skuSales: SkuSales[], data?: TotalOrderByClientData | null | undefined): Summary {
  let totalUnits = 0;
  let totalOrders = 0;
  let avgUnits = 0.00;
  skuSales.forEach(sale => {
   ...
   avgUnits = parseFloat(parseFloat(data?.avgQtyPerOrder).toFixed(2)) || 0.00;

  console.log("DATA from create Summary", {data, totalOrders, totalUnits, avgUnits} )
